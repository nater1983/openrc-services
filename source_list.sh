...
app_admin=(cgmanager metalog monit rsyslog salt sysklogd syslog-ng ulogd)
app_antivirus=(clamav)
app_crypt=(mit-krb5)
app_emulation=(libvirt virtualbox-guest-additions xe-guest-utilities)
app_misc=(lirc)
dev_db=(influxdb mysql-init-scripts postgresql redis)
dev_lang=(php)
dev_libs=(cyrus-sasl)
...
net_dns=(bind dnsmasq)
net_firewall=(iptables ufw)
net_fs=(autofs nfs-utils samba)
net_ftp=(proftpd vsftpd)
net_im=(bitlbee)
net_libs=(openslp)
net_misc=(connman dhcp dhcpcd networkmanager ntp openntpd openssh rsync teamviewer tor)
